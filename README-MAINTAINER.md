...
milestone. As title, use the current version, like _6.8.0_.
...
For all features, increase the common version (like _6.8.0_); be sure
...
`Bundle-Version` (like _6.8.0_).

For all other occurrences, except `compositeArtifacts.xml` and
`compositeContent.xml`.

Commit all with _update package version 6.8.0_.
...
```html
<p>Version <strong>6.8.0</strong> is a maintenance/new minor release; it ....</p>
...
- commit with a message like _Eclipse Embedded CDT plug-ins v6.8.0 released_.
- push the `develop` branch
...
- tag: _v6.8.0_ (with `v`)
- title _Eclipse Embedded CDT plug-ins v6.8.0_
- copy/paste from the release page
- add `[Continue reading »](https://eclipse-embed-cdt.github.io/blog/YYYY/MM/DD/plugins-v6.8.0-released/)` with a link to the web page
- do not attach files
...
```html
<p>Version <strong>6.8.0</strong> is a new major/minor/service release; it updates ...</p>
...
<ul>
 ...
 <li><a href="https://eclipse-embed-cdt.github.io/blog/YYYY/MM/DD/plugins-v6.8.0-released/">Eclipse Embedded CDT plug-ins v6.8.0 released</a>&nbsp;(for updating the plug-ins on an existing Eclipse)</li>
</ul>
```
...
- pull new commits
- edit `embedcdt.aggrcon`
  - replace location to <https://download.eclipse.org/embed-cdt/releases/6.8.0/p2/>
  - replace full version to `6.8.0.202602251544`
  - replace short version to `6.8.0`
- commit with a message like:
  - _embedcdt: update for 6.8.0_,
- push
- create pull request
...
use a subject like **Eclipse Embedded CDT plug-ins v6.8.0 released**, and
...
- using the `@EmbedCDT` account, enter a message like
  **Eclipse Embedded CDT plug-ins v6.8.0 released** and on the next line
