...
milestone. As title, use the current version, like _6.7.0_.
...
For all features, increase the common version (like _6.7.0_); be sure
...
`Bundle-Version` (like _6.7.0_).

For all other occurrences, except `compositeArtifacts.xml` and
`compositeContent.xml`.

Commit all with _update package version 6.7.0_.
...
```html
<p>Version <strong>6.7.0</strong> is a maintenance/new minor release; it ....</p>
...
- in `eclipse-plugins.git`, the master branch, edit both .xml files
  ...
  - add new child like `<child location='../../releases/6.7.0/p2'/>`
  - update `p2.timestamp` to the value shown at the end of the `make-release-candidate-from-master`
- **commit** master with a message like _add 6.7.0 to composite_
- **push** master
...
- commit with a message like _Eclipse Embedded CDT plug-ins v6.7.0 released_.
- push the `develop` branch
...
- tag: _v6.7.0_ (with `v`)
- title _Eclipse Embedded CDT plug-ins v6.7.0_
- copy/paste from the release page
- add `[Continue reading »](https://eclipse-embed-cdt.github.io/blog/YYYY/MM/DD/plugins-v6.7.0-released/)` with a link to the web page
- do not attach files
...
```html
<p>Version <strong>6.7.0</strong> is a new major/minor/service release; it updates ...</p>
...
<ul>
 ...
 <li><a href="https://eclipse-embed-cdt.github.io/blog/YYYY/MM/DD/plugins-v6.7.0-released/">Eclipse Embedded CDT plug-ins v6.7.0 released</a>&nbsp;(for updating the plug-ins on an existing Eclipse)</li>
</ul>
```
...
use a subject like **Eclipse Embedded CDT plug-ins v6.7.0 released**, and
...
- using the `@EmbedCDT` account, enter a message like
  **Eclipse Embedded CDT plug-ins v6.7.0 released** and on the next line
